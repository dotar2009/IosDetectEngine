...
# 20190717增加scp方法来传输大文件，解决sftp无法传输的问题
def file_transfer(ip, username, password, remote_file, local_path):
    passwd_key = '.*ssword.*'
    cmd = "scp {}@{}:{} {}".format(username, ip, remote_file, local_path)
    try:
        child = pexpect.spawn(cmd)
        child.expect(passwd_key)
        child.sendline(password)
        child.expect(pexpect.EOF, timeout=None)
        while not child.isalive():
            print "scp transfer finished"
            break
        return True
    except Exception as exe:
        print "scp transfer error"
    ...
    def sftp_get(ip, port, username, password, remote_file, local_path):
        ...
        while True:
            try:
                ...
                transport.close()
                time.sleep(5)
                data.logger.debug("SFTP GET FILE Error,try scp transfer")
                # cmd = "sftp -P {} {}@{}:{} {}".format(port, username, ip, remote_file, local_path)
                # out = commands.getstatusoutput(cmd)
                # if out[0] == 0:
                #     break
                flag = file_transfer(ip,username,password,remote_file,local_path)
                if flag:
                    break
